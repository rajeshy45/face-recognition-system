...
# checking if images folder exists or not
if not os.path.isdir(image_dir):
    os.mkdir(image_dir)

try:
    name_class = input("Input name of the person: ")  # name of person for recognition
except:
...
pin = (
    sorted([int(n[: n.find(".")]) for n in os.listdir(path) if n[0] != "."] + [0])[-1]
    + 1
)
...
# Beginning message
print(
    f"The program will save {count_max} samples. Move your head around to increase while it runs\n"
)
